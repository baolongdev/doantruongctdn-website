...
function EditorsPick() {
  const [editorsPick, SetEditorsPick] = useState<PopularPostProps>([]);

  async function editorsPickGet() {
    const slug = "posts/setting/EditorsPick";
    const fields = "data";
    const apiEndpoint = `${urlbase}/api/file/readfile?slug=${slug}&fields=${fields}`;
    const res = await fetch(apiEndpoint);
    const raws = await res.json();
    SetEditorsPick(raws.data);
  }
  useEffect(() => {
    editorsPickGet();
  }, []);
  ...
  return (
    <div className="mb-8">
      <h2 className={``}>Được biên tập viên lựa chọn</h2>
      <h4 className="h4 font-bold leading-snug tracking-tight mb-4">Biên tập viên chọn</h4>
      <ul className="-my-2">
